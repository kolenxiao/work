...
function jumpPage(no) {
	url = 'planItemManage!listUnSelectedDpType.action?';
...
function doChange(obj) {
	...
	url = 'planItemManage!listUnSelectedDpType.action?limit=' + pageSizeVal;
	//表单提交
...
function doQuery(){
	...
	searchForm.action = "planItemManage!listUnSelectedDpType.action";
	searchForm.submit();
}
...
function doAddPlanDpType(){
	...
	if(confirm("确认关联分类？")){
		$.post("planItemManage!addPlanItem.action",{"planId":planId, "itemIds": itemIdArray.join(","), "operFlag":"addDpType"},function(data){
			if("success" == data){
				alert("操作成功");
				doQuery();
			}else{
...
<div id="position">
  <p><s:text name="sdp.sce.dp.admin.global.label.current.position"/>方案管理&gt; 方案未关联固定分类列表</p>
  <div id="pright"></div>
</div>
...
<div id="searchbar">
  <div id="search_itemlist">
		<s:form id="searchForm" name="searchForm" action="" method="post" cssStyle="margin:0" theme="simple" >
			...
			<ul>
				<li>分类名称：
					<s:textfield id="typeName"
								name="typeName" size="15" maxlength="64"
								value="%{typeName}" onblur="return this.value = this.value.replace(/(^\s*)|(\s*$)/g, '');" />
				</li>
				<li>
					<input type="submit" onclick="doQuery()" value="" class="btn_sendData"  />&nbsp;
				</li>
			</ul>
		</s:form>
	</div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr >
			...
			<td><a class="detail_link" href="javascript:viewDeatil('<s:property value="id"/>')" >
						<s:if test="%{typeName.length() > 12}">
		               		<s:property value="%{typeName.substring(0, 12) + \"..\"}"/>
		               	</s:if>
		               	<s:else>
...
<div class="btnlistbar">
	<s:if test="#session.userResMap.addPlanItem!=null">
		<input class="inputstyle" onclick="doAddPlanDpType()" type="button"  value="<s:text name="sdp.sce.dp.admin.global.btn.confirm" />"/>
	</s:if>
