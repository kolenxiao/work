...
function jumpPage(no) {
	url = 'planItemManage!listUnSelectedSelf.action?';
...
function doChange(obj) {
	...
	url = 'planItemManage!listUnSelectedSelf.action?limit=' + pageSizeVal;
	//表单提交
...
function doQuery(){
	...
	searchForm.action = "planItemManage!listUnSelectedSelf.action";
	searchForm.submit();
}
...
function doAddSelfType(){
	...
	if(confirm("确认关联分类？")){
		$.post("planItemManage!addPlanItem.action",{"planId":planId, "itemIds": itemIdArray.join(","), "operFlag":"addSelfType"},function(data){
			if("success" == data){
				alert("操作成功");
				doQuery();
			}else{
...
<div id="position">
  <p><s:text name="sdp.sce.dp.admin.global.label.current.position"/>方案管理&gt; 方案未关联自定义分类列表</p>
  <div id="pright"></div>
</div>
...
<div id="searchbar">
  <div id="search_itemlist">
		...
		<ul>
			<li>分类名称：
				<s:textfield id="name"
							name="name" size="15" maxlength="64"
							value="%{name}" onblur="return this.value = this.value.replace(/(^\s*)|(\s*$)/g, '');" />
			</li>
			<li>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr >
			...
			<td><s:if test="%{parentTypeCode == 'APP_TYPE'}">应用</s:if>
	            <s:elseif test="%{parentTypeCode == 'GAME_TYPE'}">游戏</s:elseif>
				<s:else>无</s:else></td>
		</tr>
		</s:iterator>
	</table>
...
<div class="btnlistbar">
	<s:if test="#session.userResMap.addPlanItem!=null">
		<input class="inputstyle" onclick="doAddSelfType()" type="button"  value="<s:text name="sdp.sce.dp.admin.global.btn.confirm" />"/>
	</s:if>
