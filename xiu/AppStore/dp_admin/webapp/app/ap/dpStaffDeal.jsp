...
//提交表单
function doSave(){
	var formName= document.getElementById("checkForm");
...
<div id="position">
  <p>
  <s:text name="sdp.sce.dp.admin.dptype.pro" />：<s:text name="sdp.sce.dp.admin.dpstaff" /> &gt; <s:text name="sdp.sce.dp.admin.dpstaff.audit" /></p>
  <div id="pright"></div>
</div>
...
<div class="view_nav"  id="nav05" >
	<span class="float_l">审核AP信息</span>
	<span class="icon_down" onclick="ShowHideBox(this,'box5','nav05')" ></span>
</div>
<div id="box5">
		<div class="data_view">
			<form  id="checkForm" name="checkForm" action="<s:property value="#session.userResMap.saveCheck.url"/>?flag=<%=request.getParameter("Flag") %> &id=${dpStaff.id }" method="post">
				<s:token/>
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
				   		<th scope="row"><s:text name="sdp.sce.dp.admin.dpstaff.audit.opinion" /></th>
				    	<td width="90%"><span><input name="dpStaff.staffStatus" type="radio"   value="1003"/> <s:text name="sdp.sce.dp.admin.ap.state.pass" /></span><span style="margin-left:30px;"><input name="dpStaff.staffStatus" type="radio" checked value="1001" /><s:text name="sdp.sce.dp.admin.ap.state.notpass" /> </span></td>
				    </tr>
				  	<tr>
				    	<th scope="row"><s:text name="sdp.sce.dp.admin.dpstaff.remarks" /></th>
				    	<td width="90%"><textarea id="auditOption" name="dpAuditRecord.auditOption" cols="80" rows="4"valid="required|limit" min="0" max="200" errmsg="dp_audit_remake_notEmpty|dp_audit_remake_lenLess200"></textarea> <span class="style-red" id="error">*</span>
				    	<span id="errMsg_dpAuditRecord.auditOption" style="color:#FF0000"></span>
				    	</td>
				  	</tr>
				</table>
			  	<div class="btnlistbar">
			  		<input class="inputstyle" value="<s:text name="sdp.sce.dp.admin.global.btn.save" />"  type="button" onclick="doSave()"/>
