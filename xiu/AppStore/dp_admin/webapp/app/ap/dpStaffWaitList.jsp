   ...
   function doDelete() {
	   var idArray = new Array();
		...
		if (idArray.length > 0) {
			...
			var parm ={'ids':idVal};
			$("#dialog").remove();
			dialogList('<s:text name="sdp.sce.dp.admin.global.inform" />',300,150,'<s:text name="sdp.sce.dp.admin.global.del.data.confirm" />',0,0,2,this);
			//注意此处的dialog_btn_conform为"确定"按钮的id,点击"确定"时执行删除操作
			$("#dialog_btn_conform").click(function()
			 {
				$.ajax({
				    url: 'dpStaff!doDelete.action',
				    type: 'POST',
				    data: parm,
				    dataType: 'json',
				    error: function(msg){
				    ...
				    success: function(response){
						...
						if (dataObj && dataObj.success) {
				    		location.href = "dpStaff!doList.action?flag=1&limit="+pageSizeVal;
						...
						}else if (dataObj && dataObj.exception)
						{
							...
							dialogList('<s:text name="sdp.sce.dp.admin.global.inform" />',300,150,'<s:text name="sdp.sce.dp.admin.dpstaff.ap.cannont.delete" />',0,0,1,this);
						}
				    }
				  });
					  });
	...
	function searchDp(startTimeId,endTimeId) {
		...
		url = 'dpStaff!doList.action?flag=1&';
	...
	function doChange(obj) {
		...
		url = 'dpStaff!doList.action?flag=1&limit='
			+ pageSizeVal;
		//表单提交
...
<body id="cnt_body">
	<div id="position">
		<p><s:text name="sdp.sce.dp.admin.dptype.pro" />：<s:text name="sdp.sce.dp.admin.dpstaff" /> &gt;<s:text name="sdp.sce.dp.admin.dpstaff.ap.information.audit.to.wait" /></p>
		<div id="pright"></div>
	</div>
    <div id="searchbar">
	  		...
	  		<s:form id="searchForm" name="searchForm" action="" method="post" cssStyle="margin:0" theme="simple" >
				   ...
				   <li>
					...
					<li >
						...
						<img onclick="WdatePicker({el:'beginDpStaffCtime',dateFmt:'yyyy-MM-dd'})"
					...
					<li>
						...
						<s:textfield id="endDpStaffCtime"
								...
								value="%{dpStaff.endDpStaffCtime}"  readonly="true" /></li><li>
	...
	<div class="btnbar">
		<ul>
	      	<s:if test="#session.userResMap.deleteAp !=null">
		 		<li>
		 			<a href="#" onclick="doDelete()"><b><s:text name="sdp.sce.dp.admin.global.btn.del" /></b></a>
				</li>
			</s:if>
		</ul>
	</div>
		...
		<tr>
		    ...
		    <td class="editbar">
		    	<s:if test="#session.userResMap.queryApInfo!=null">
			    	<a href="<s:property value="#session.userResMap.queryApInfo.url"/>?forward=audit&Flag=1&id=<s:property value='id' />"
					   target="mainFrame" class="detail_link">&nbsp;&nbsp;<s:text name="sdp.sce.dp.admin.log.audit.operate" />&nbsp;&nbsp;</a>
				</s:if>
			</td>
		</tr>
		</s:iterator>
