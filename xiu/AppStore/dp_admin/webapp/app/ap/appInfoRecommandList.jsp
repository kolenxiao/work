...
<script type="text/javascript">
	...
	function jumpPage(no) {
		...
		url = 'appRecommend!doSearchRecommendList.action?';
	...
	function doChange(obj) {
		...
		url = 'appRecommend!doSearchRecommendList.action?limit=' + pageSizeVal;
		//表单提交
			...
			$("#dialog_btn_conform").click(
				function(){
						$.ajax({
								cache : false,
								async : false,
								url : 'appRecommend!doBatchCommend.action',
								type : 'POST',
								data : parm,
								dataType : 'json',
								error : function(msg) {
								...
								success : function(response) {
									...
									if (dataObj && dataObj.success) {
										location.href = "appRecommend!doSearchRecommendList.action?&limit="
												+ pageSizeVal;
									} else if (dataObj
											&& dataObj.msg) {
										dialogList(
	...
	function commend(obj, id, commendFlag) {
		location.href = 'appRecommend!doCommend.action?appInfo.id=' + id
				+ '&commendFlag=' + commendFlag;
	}

	function addRecommend()
	{
		action = "dpAppInfo!toAddRecommentList.action";
		var jsonObj = {
			scrollType : 'yes'
		};
...
<body id="cnt_body">
<div id="position">
  <p><s:text name="sdp.sce.dp.admin.global.label.current.position"/><s:text name="sdp.sce.dp.admin.ap.name" />&gt; 应用推荐</p>
  <div id="pright"></div>
</div>
...
<div class="databar">
	<div class="btnbar">
		<ul>
			<s:if test="#session.userResMap.cancleRecommendApp!=null">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr >
			...
			<td class="editbar">
				...
				<s:form method="post" id="%{id}sortForm" action="appRecommend!doRecommendSort.action" name="%{id}sortForm" theme="simple">
					<s:hidden name="limit" id="limit" value="%{limit}"></s:hidden>
					<s:hidden name="appRecommend.id" id="appRecommend.id" value="%{id}"></s:hidden>
