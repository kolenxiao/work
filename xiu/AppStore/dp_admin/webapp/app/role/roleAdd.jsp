...
<title><s:text name="sdp.sce.dp.admin.role.add.role"/></title>
...
validator = $("#uspRoleForm").validate({
	...
	},'<font color="red">角色名不能为空！</font>');

	$.validator.addMethod("checkRoleName",function(){
		...
		$.ajax({
	         ...
	         async:false,
	         type:"POST",
		   	 	url: "role!isRoleNameSame.action",
		   	 	data: parm,
		   	 dataType: 'json',
			    ...
			    success: function(response){
					...
					if (dataObj && dataObj.msg) {
			    		result = false;
			    		$("#findRoleNameError").append(dataObj.msg);
					}
			    }
		   	});
			return result;
...
<div id="position">
  <p><s:text name="sdp.sce.dp.admin.global.label.current.position"/><s:text name="sdp.sce.dp.admin.role"/> &gt; <s:text name="sdp.sce.dp.admin.role.add.role"/>
  <div id="pright"></div>
</div>
<div class="view_nav"><s:text name="sdp.sce.dp.admin.role.add.role"/></div>
<s:form id="uspRoleForm" action="role!addRole.action" method="post">
<s:token/>
<div class="data_view">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
    <th scope="row"><s:text name="sdp.sce.dp.admin.role.roleName"/></th>
    <td><input id="roleName" name="role.name" type="text" /><span class="must_input">*</span><font color='gray'>&nbsp;由字母、数字、下划线、中文、空格组成</font>
    <span id="findRoleNameError" style="color:red;"></span>
    </td>
  </tr>
  <tr>
  <th scope="row"><s:text name="sdp.sce.dp.admin.role.roleDescription"/></th>
    <td><textarea name="role.description" cols="100" rows="4"></textarea><span class="notes"></span></td>
  </tr>
  <tr>
